...
if(isset($_POST) && isset($_POST['delete'])) {
    $Response = $ToDo->deleteEntry($_POST);
    $_POST = array();
}
...
<script>
    function editData(id) {
        ...
        fetch(URL, fetchData)
            ...
            .then(function (data) {
               document.querySelector('#entry').value = data.data[0].id;
               document.querySelector('#editHeadline').value = data.data[0].headline;
               document.querySelector('#editDescription').innerText = data.data[0].description;
               myModal.toggle();
            });
        //e.preventDefault();
    }
